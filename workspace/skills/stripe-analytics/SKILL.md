...
description: |
  ...
  Use when user asks about: "Stripe metrics", "MRR", "churn", "top customers", 
  "revenue forecast", "customer LTV", "Stripe dashboard", "SaaS analytics",
  "churn risks", "at-risk customers", "revenue by country/plan"
  
  Supports multiple projects: skysnail, anderro, viralsky, foodient
  
  REQUIRES: STRIPE_{PROJECT}_READ_KEY (restricted key with read:customers/subscriptions/invoices/payment_intents)
---
...
**Project selection**: If only one key exists, uses it automatically. If multiple, requires explicit project name.
...
# OR for multiple projects (recommended)
STRIPE_SKYSNAIL_READ_KEY=sk_read_xxx...
STRIPE_ANDERRO_READ_KEY=sk_read_xxx...
STRIPE_VIRALSKY_READ_KEY=sk_read_xxx...
STRIPE_FOODIENT_READ_KEY=sk_read_xxx...
```
...
### Project Selection
- **Auto**: If only one key exists, uses it automatically
- **Explicit**: Specify `"project": "skysnail"` in input
- **Slash commands**: `/stripe-metrics skysnail` or `/stripe-metrics anderro`
